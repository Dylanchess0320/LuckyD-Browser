...
class TestVersionUnification:
    ...
    def test_versions_unified_at_current(self):
        ...
        assert "filevers=(10, 4, 0, 0)" in version_info
        assert "prodvers=(10, 4, 0, 0)" in version_info

        iss = (_BROWSER_DIR / "installer" / "LuckyDBrowser.iss").read_text(encoding="utf-8")
        assert '#define AppVersion   "10.4.0"' in iss
        # The output filename + setup version resources are separate
        # hardcodes (regression: 10.2.1 first shipped a Setup-10.2.0.exe).
        assert "OutputBaseFilename=LuckyDBrowserSetup-10.4.0" in iss
        assert "VersionInfoVersion=10.4.0.0" in iss
        assert "VersionInfoProductVersion=10.4.0.0" in iss
