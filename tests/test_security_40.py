...
class TestVersionUnification:
    ...
    def test_versions_unified_at_current(self):
        ...
        assert browser.__version__ == "9.7.0"

        with open(_REPO_ROOT / "pyproject.toml", "rb") as f:
            assert tomllib.load(f)["project"]["version"] == "9.7.0"

        ai_bridge = (_BROWSER_DIR / "browser_core" / "ai_bridge.py").read_text(encoding="utf-8")
        assert "LuckyDBrowser/9.7" in ai_bridge
        assert "LuckyDBrowser/1.0" not in ai_bridge

        version_info = (_BROWSER_DIR / "version_info.txt").read_text(encoding="utf-8")
        assert "9.7.0.0" in version_info
        # "9.7.0.0" contains the substring "6.0.0", so the stale-version guard
        ...
        assert "filevers=(9, 7, 0, 0)" in version_info
        assert "prodvers=(9, 7, 0, 0)" in version_info

        iss = (_BROWSER_DIR / "installer" / "LuckyDBrowser.iss").read_text(encoding="utf-8")
        assert '#define AppVersion   "9.7.0"' in iss
