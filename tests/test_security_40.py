...
class TestVersionUnification:
    ...
    def test_versions_unified_at_current(self):
        ...
        assert browser.__version__ == "10.2.0"

        with open(_REPO_ROOT / "pyproject.toml", "rb") as f:
            assert tomllib.load(f)["project"]["version"] == "10.2.0"
        ...
        assert "10.2.0.0" in version_info
        ...
        assert "filevers=(10, 2, 0, 0)" in version_info
        assert "prodvers=(10, 2, 0, 0)" in version_info

        iss = (_BROWSER_DIR / "installer" / "LuckyDBrowser.iss").read_text(encoding="utf-8")
        assert '#define AppVersion   "10.2.0"' in iss
