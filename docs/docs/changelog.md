...
Provider-list release: AI provider list (`lucky-code providers`, `/providers`,
`GET /api/providers`), offline-proof model-resolver cache, honest cost tracking
(deepseek-v4 + free-tier $0), Agent Mesh 1.0.1 (Gemini CLI → Antigravity).
Shipped as `LuckyDBrowserSetup-9.2.0.exe`.

## [9.1.0] - 2026-09-15

Agent-terminals upgrade: Gemini CLI + Cline CLI first-class in both Agent 1
(v3.6) and Agent 2 (v2.2) terminals, every free model in `/model`
(Gemini free tier, Cline free tier x12, new ClinePass glm-5.3/glm-5.2/
qwen3.8-max), `gemini`/`cline` provider aliases. Shipped as
`LuckyDBrowserSetup-9.1.0.exe`.
...
- **Smart model routing** — `core/router.py`'s ModelRouter is wired into `AIBridge.chat()` auto mode: it picks the provider per question (explicit picks always win; non-viable picks fall back to the existing chain).
