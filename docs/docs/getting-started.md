...
```powershell
LuckyDBrowserSetup-9.0.0.exe /VERYSILENT /NORESTART
```
...
Install the new `LuckyDBrowserSetup-9.0.0.exe` right over your existing install — profile, settings, and workspaces carry over. The browser also checks for updates itself and can install them in-app with one click.
