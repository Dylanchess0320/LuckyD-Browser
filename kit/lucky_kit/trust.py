"""Trust foundation — "agentic with receipts", extracted from LuckyD.
...
Three pieces, zero dependencies beyond the standard library:
  - Permission *scopes*: tools grouped into human-meaningful capabilities
    ("Run shell commands", "Write & edit files", ...) instead of a flat
    tool list. Powers "what the agent can touch" UIs.
...
Storage lives under ~/.lucky-kit (LUCKY_KIT_DATA_DIR overrides, used by tests).

Build with LuckyD: wrap your own agent's tool calls with ``scope_of``,
``risk_of``, ``redact_args``, ``AuditLog.record`` and ``TrustPolicy`` and
you get LuckyD-grade trust without running LuckyD.
"""
...
def _data_dir() -> Path:
    override = os.environ.get("LUCKY_KIT_DATA_DIR")
    base = Path(override) if override else Path.home() / ".lucky-kit"
...
# Add your own tools here — the mapping is yours to extend.
...
#: Public view of explicit tool -> scope overrides (extend at runtime).
...
# Standalone risk table: no LuckyD internals needed.
# high   — irreversible or privileged: shell, desktop control, push, agents
# medium — state-changing but recoverable: file writes, browser control, MCP
# low    — read-only: read, network fetch, memory recall, system utilities
_RISK_BY_SCOPE: dict[str, str] = {
    "shell": "high",
    "desktop": "high",
    "git": "high",
    "agents": "high",
...
def risk_of(tool_name: str) -> str:
    """low / medium / high — derived from the tool's scope."""
    return _RISK_BY_SCOPE.get(scope_of(tool_name), "medium")
