...
See [`kit/SKILL_AUTHORING.md`](kit/SKILL_AUTHORING.md) to publish your own skill to the LuckyD marketplace.

---

## Latest release

**v10.1.0** — Google AI Pro edition: OpenCode is back in the Agent Mesh, Gemini refreshed, Nano Banana + Veo media generation, 2TB Drive backup:

| | |
|---|---|
| **⬇ Windows installer** | [`LuckyDBrowserSetup-10.1.0.exe`](https://github.com/Dylanchess0320/LuckyD-Browser/releases/download/v10.1.0/LuckyDBrowserSetup-10.1.0.exe) — built on a real Windows runner with Inno Setup 6. Per-user, no admin, silent-install flags. |
| **🔵 OpenCode restored** | `mesh-opencode` is back in the Agent Mesh — keyed via `OPENCODE_API_KEY`, alongside MiniMax and Cline. (The old free tier is gone; keyed only.) |
| **✨ Gemini refreshed** | Gemini 2.5 Pro/Flash, 3 previews, current free-tier model list. |
| **🎨 Nano Banana + Veo** | Image generation (`browser/models/gemini_media.py`) and Veo 3.1 video generation through the Gemini API. |
| **💾 2TB Drive backup** | One-command backup to Google Drive (`browser/browser_core/drive_backup.py`). |

**v9.9.0** — Anti-block hardening: no more `-ExecutionPolicy Bypass` in the installer, portable ZIP for locked-down PCs, code-signing wired in:

| | |
|---|---|
| **⬇ Windows installer** | [v9.9.0 release page](https://github.com/Dylanchess0320/LuckyD-Browser/releases/tag/v9.9.0) — built on a real Windows runner with Inno Setup 6. Per-user, no admin, silent-install flags. |
...
| **🖥 MiniMax in the mesh** | `mesh-mcode`/`mesh-mmx` in the Agent Mesh; Cline session is the free default. |
...
| | |
|---|---|
| **🔬 Honest Deep Research** | Live engine chip (resolved provider + model + backend), keyed backends fail fast with `no API key configured`, evidence from `evidence.json`, honest cancel with no overlap. |
| **💰 Opt-in contributor tier** | `muse-spark-1.3` standard vs `muse-spark-1.3-contributor` (cheaper because the provider may train on prompts). Hidden unless `/contributor on` — honest cached-token cost tracking. |

</details>

<details>
<summary>New in 9.5.0 — Terminal crash fix</summary>

A Bash tool call with a missing/null `command` raised `AttributeError` and killed the agent's terminal session — the shell tools now validate input and return an error result instead, and the timeout kill path is regression-tested.

</details>

<details>
<summary>New in 9.4.0 — Antigravity replaces Gemini</summary>
...
Silent: `LuckyDBrowserSetup-10.1.0.exe /VERYSILENT /NORESTART`
